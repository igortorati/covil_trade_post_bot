...
export default class DeleteSeasonCommand implements Command {
  ...
  public async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    const temporada = await Seasons.findOne({ where: { season, is_deleted: false } });

    if (temporada) {
      await temporada.update({ is_deleted: true, is_current: false });
      await interaction.reply({ content: `Temporada ${season} desativada com sucesso.`, flags: ["Ephemeral"] });
    } else {
      await interaction.reply({ content: "Temporada não encontrada ou já está desativada.", flags: ["Ephemeral"] });
    }
  }
