import {
  ...
  export default class SetCurrentSeasonCommand implements Command {
    ...
    public async execute(interaction: ChatInputCommandInteraction): Promise<void> {
      ...
      const currentSeason = await Seasons.findOne({ where: { is_current: true } });
  
      if (currentSeason) {
        await currentSeason.update({ is_current: false });
      }
  
      const seasonEntry = await Seasons.findOne({ where: { season: seasonName, is_deleted: false } });
      if (seasonEntry) {
        await seasonEntry.update({ is_current: true });
        await interaction.reply({ content: `Temporada ${seasonName} definida como a temporada atual.`, flags: ["Ephemeral"] });
      } else {
