  ...
  public async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    const requests = await TradeRequests.findAll({
      ...
      include: [
        ...
        { model: Items, as: "itemOffered", include: [Rarities] },
        { model: Characters },
      ],
      order: [["id", "ASC"]],
    });
    ...
    if (requests.length === 0) {
      await interaction.reply({
        content: "Nenhuma requisição de troca encontrada.",
      });
      return;
    }

    const createEmbed = (req: TradeRequests) => {
      ...
      return new EmbedBuilder()
        ...
        .setTitle(`🔁 Solicitação de Troca #${req.id}`)
        .addFields(...fields);
    };
